...
// 分页进 URL query：返回列表、刷新、分享链接都保持页码
...
async function importText() {
  ...
  message.value = ''
  try {
    const data = await api<{ filename: string; total: number; inserted: number; skipped_duplicates: number; group_name: string }>('/api/json-files/import-text', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ content, filename: textFilename.value.trim() || undefined }),
    })
    message.value = `导入成功：${data.inserted} 条入提示词库，跳过 ${data.skipped_duplicates} 条重复（分组「${data.group_name}」，已存为 ${data.filename}）`
    messageType.value = 'success'
    textContent.value = ''
    textFilename.value = ''
...
<template>
  ...
  <div class="toolbar">
    <label class="upload-btn">
      ...
      {{ uploading ? '导入中...' : '导入 JSON 文件' }}
      <input type="file" accept=".json,application/json" @change="upload" />
    </label>
    <button class="btn btn-secondary" @click="showTextImport = !showTextImport">
      {{ showTextImport ? '收起粘贴导入' : '粘贴 JSON 导入' }}
    </button>
  ...
  <div v-if="showTextImport" class="card text-import-card">
    <div class="card-body">
      <div class="text-xs muted" style="margin-bottom:8px;">
        直接粘贴一段提示词 JSON（格式：数组，或 {"entries":[...]}，每条需含 positive 字段）。导入后会自动创建备份文件保存到项目 data/json/ 目录。
      </div>
      <textarea
        v-model="textContent"
        class="input json-textarea"
        rows="8"
        placeholder='[{"title": "穿汉服的女孩", "positive": "a girl in hanfu, master piece"}]'
      ></textarea>
      <div class="text-import-actions">
        <input
          v-model="textFilename"
          class="input filename-input"
          placeholder="文件名（可选，默认：手动导入-年月日时分秒.json）"
        />
        <button class="btn btn-primary" :disabled="importingText" @click="importText">
          {{ importingText ? '导入中...' : '导入' }}
        </button>
      </div>
    </div>
  </div>
...
.text-import-card { margin-bottom: 12px; }
.json-textarea { width: 100%; font-family: var(--font-mono, ui-monospace, monospace); font-size: 12.5px; resize: vertical; }
.text-import-actions { display: flex; gap: 8px; margin-top: 10px; flex-wrap: wrap; }
.filename-input { flex: 1; min-width: 220px; }
</style>
