...
<template><PageHeader eyebrow="PROMPT DATA" title="JSON 文件" description="上传提示词数据并查看生成状态。" /><section class="toolbar"><label class="upload-button">导入 JSON<input type="file" accept=".json,application/json" @change="upload" /></label><span class="muted">{{ message }}</span></section><section class="panel"><div v-if="!files.length" class="muted">暂无 JSON 文件。</div><div v-for="file in files" :key="file.id" class="list-row"><strong>{{ file.filename }}</strong><span>{{ file.completed_count }} / {{ file.total_count }} 已完成 · {{ file.failed_count }} 失败</span></div></section></template>
